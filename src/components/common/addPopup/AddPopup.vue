...
import { addBillItem, updateBillItem } from '@/network/bill'
...
let { showAddRead, defaultData } = defineProps<{
  showAddRead: Boolean
...
const showDateFn = () => {
  ...
  dateType.value = 'date'
  dateTitle.value = '选择年月日'
}

// 每次打开都初始化
watch([showAdd, showAddEdit], () => {
  setTimeout(() => {
    // 是详情页
    if (Object.keys(defaultData).length > 0) {
      ...
      selectedAddDate.value = new Date(Number(defaultData.date || ''))
      remark.value = defaultData.remark || ''
      type_pay.value = defaultData.pay_type === 1 ? 'expense' : 'income'
    } else {
      // 是添加页
      amount.value = ''
      selectedAddDate.value = new Date()
      remark.value = ''
...
const addOrUpdateBillItemRequest = async () => {
  const params: BillItem = {
    ...
    date: Number(selectedAddDate.value) + '',
...
<template>
  <!-- 增加页面的弹出框 -->
  <van-popup v-model:show="showAddRead" position="bottom" round>
    <div class="add-page">
      <header>
        <!-- 支付/收入 -->
        <div class="type">
        ...
        <div class="date">
          <span @click="showDateFn">
            {{ dayjs(selectedAddDate).format('YYYY-MM-DD') }}
    ...
    <TotalDate
      :showDateRead="showAddDate"
      :selectedDateRead="selectedAddDate"
